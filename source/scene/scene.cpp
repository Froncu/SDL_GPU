...
namespace fro
{
   Scene::Scene(SDL_GPUDevice& gpu_device, std::filesystem::path const& path)
   {
      ...
      std::ranges::copy(indices,
         static_cast<decltype(indices)::value_type*>(SDL_MapGPUTransferBuffer(&gpu_device, index_transfer_buffer.get(),
            false)));
      ...
      std::ranges::copy(vertices,
         static_cast<decltype(vertices)::value_type*>(
            SDL_MapGPUTransferBuffer(&gpu_device, vertex_transfer_buffer.get(), false)));
