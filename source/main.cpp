...
{
   static_cast<Application*>(app_state)->tick();
   return SDL_APP_CONTINUE;
}
catch (std::exception& exception)
{
...
{
   return
      static_cast<Application*>(app_state)->process_event(*event)
         ? SDL_APP_CONTINUE
         : SDL_APP_SUCCESS;
}
catch (std::exception& exception)
...
{
   delete static_cast<Application*>(app_state);
